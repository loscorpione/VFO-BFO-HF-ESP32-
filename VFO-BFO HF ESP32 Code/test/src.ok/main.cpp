...
unsigned long lastSMeterUpdate = 0;
const int SMETER_UPDATE_INTERVAL = 200; // ms
...
void loop() {
  ...
  // Aggiorna S-meter
if (millis() - lastSMeterUpdate > SMETER_UPDATE_INTERVAL) {
  // Leggi e media multiple letture per ridurre il rumore
  int sMeterValue = 0;
  for (int i = 0; i < SMETER_SAMPLES; i++) {
    sMeterValue += analogRead(SMETER_PIN);
    delay(1);
  }
  sMeterValue /= SMETER_SAMPLES;
  
  updateSMeter(sMeterValue);
