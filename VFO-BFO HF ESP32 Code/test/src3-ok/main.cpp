...
void setup() {
  ...
  // Inizializzazione BFO
  setupBFO();  // Aggiungi questa linea
  drawBFOArea();  // Aggiungi questa linea
  updateBFODisplay();  // Aggiungi questa linea
...
void loop() {
  ...
  if (digitalRead(SW_MODE) == LOW && !modeButtonPressed) {
    if (millis() - lastModeButtonPress > buttonDebounce) {
      ...
      updateModeOutputs();
      updateModeInfo();
      updateBFO();  // Aggiungi questa linea per aggiornare il BFO alla nuova modalità
      updateBFODisplay();  // Aggiungi questa linea per aggiornare la visualizzazione del BFO
      delay(300);
    }
  }
  ...
  // Aggiornamento BFO
  if (millis() - lastBFOUpdate > BFO_UPDATE_INTERVAL) {
  int currentBFOValue = analogRead(BFO_PIN);
  
  if (abs(currentBFOValue - lastBFOValue) > BFO_UPDATE_THRESHOLD) {
    updateBFO();
    updateBFODisplay();  // Ora questa funzione è in display.cpp
    lastBFOValue = currentBFOValue;
  }
