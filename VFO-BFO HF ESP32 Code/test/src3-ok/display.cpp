...
void drawBFOArea() {
  // Disegna l'area del BFO solo se non siamo in modalità AM
  if (currentMode != MODE_AM) {
    // Riquadro BFO (in basso)
    tft.fillRoundRect(155, 135, 100, 25, 3, BACKGROUND_COLOR);
    tft.drawRoundRect(155, 135, 100, 25, 3, BORDER_COLOR);
  }
}

void updateBFODisplay() {
  // Se siamo in modalità AM, nascondi completamente la visualizzazione del BFO
  if (currentMode == MODE_AM) {
    tft.fillRect(155, 135, 100, 25, BACKGROUND_COLOR); // Nascondi l'area del BFO
    return;
  }
  
  int bfoOffset = getBFOOffset();
  
  tft.fillRect(160, 140, 100, 20, BACKGROUND_COLOR);
  tft.setTextColor(TFT_WHITE, BACKGROUND_COLOR);
  tft.setTextSize(1);
  
  String bfoStr = "BFO: ";
  if (bfoOffset >= 0) bfoStr += "+";
  bfoStr += String(bfoOffset) + " Hz";
  
  tft.drawString(bfoStr, 160, 140);
}